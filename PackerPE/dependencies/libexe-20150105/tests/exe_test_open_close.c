 ...
 */
int exe_test_single_open_close_file(
     ...
     int expected_result )
{
	...
	static char *function    = "exe_test_single_open_close_file";
	int result               = 0;
 ...
 * Returns 1 if successful, 0 if not or -1 on error
 */
int exe_test_multi_open_close_file(
     libcstring_system_character_t *filename,
     int access_flags,
     int expected_result )
{
	libcerror_error_t *error = NULL;
	libexe_file_t *file     = NULL;
	static char *function    = "exe_test_multi_open_close_file";
	int result               = 0;
...
#endif
{
	...
	if( exe_test_single_open_close_file(
	     argv[ 1 ],
	     LIBEXE_OPEN_READ,
	     1 ) != 1 )
	{
	...
	if( exe_test_multi_open_close_file(
	     argv[ 1 ],
	     LIBEXE_OPEN_READ,
	     1 ) != 1 )
	{
