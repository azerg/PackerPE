      ...
      format_string )
{
	...
	char error_string[ PYEXE_ERROR_STRING_SIZE ];
	char exception_string[ PYEXE_ERROR_STRING_SIZE ];

	static char *function     = "pyexe_error_raise";
	...
	if( error != NULL )
	{
		...
		{
			while( error_string_index < PYEXE_ERROR_STRING_SIZE )
			{
			...
			if( error_string_index >= PYEXE_ERROR_STRING_SIZE )
			{
				error_string[ PYEXE_ERROR_STRING_SIZE - 1 ] = 0;
			}
			PyErr_Format(
			 exception_object,
			 "%s %s",
